...
return [
    ...
    // Off unless explicitly enabled: the client POSTs synchronously on terminate,
    // so an unset token would cost every request a timeout against a dead host.
    'enabled' => env('LARAOWL_ENABLED', false),

    // Application Token and Server URL
    'token' => env('LARAOWL_TOKEN'),
    'server_url' => env('LARAOWL_SERVER_URL', 'https://watch.mellor.pizza'),
    ...
    'ingest' => [
        // Caps how long an FPM worker is held after the response is sent.
        'timeout' => (float) env('LARAOWL_INGEST_TIMEOUT', 1.0),
